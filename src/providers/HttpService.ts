...
import {Response, Headers, RequestOptions, URLSearchParams, RequestOptionsArgs} from '@angular/http';
...
export class HttpService {
  ...
  public postAuthData(url: string, paramMap?: any,authHeaders?:any): Observable<Response> {
    let headers = new Headers({
      'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
      'Accept': 'application/json;charset=utf-8'
    }); 
    for (var index = 0; index < authHeaders.length; index++) {
        var element = authHeaders[index];
        for (let key in element) {
          headers.append(key,element[key]);
          console.log("postAuthData:"+element[key]);
        }
    }
    //console.log(JSON.stringify(headers).toString());
    return this.http.post(url, HttpService.buildURLSearchParams(paramMap).toString(), new RequestOptions({headers:headers}))
  }
